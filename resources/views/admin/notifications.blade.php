...
<div class="panel">
    ...
    @forelse($notifications as $notification)
        <div style="padding:16px 0; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:10px;
                    {{ $notification->is_read ? '' : 'background:#f0f4ff; border-radius:10px; padding:16px; margin-bottom:8px;' }}">
...
@if($notifications->where('is_read', false)->count() > 0)
    <form action="{{ route('admin.notifications.readAll') }}" method="POST" style="margin-top:10px;">
        @csrf
        @method('PATCH')
